...
async fn expensive_llm_call(prompt: &str, model: &str) -> Vec<Generation> {
    ...
    println!("🤖 LLM Call: '{prompt}' -> '{response}' (model: {model})",);
...
async fn basic_cache_example() {
    ...
    if result.is_none() {
        ...
        cache
            .aupdate(prompt, llm_string, generations)
            .await
            .unwrap();
...
async fn performance_example() {
    ...
    println!("Results:");
    println!(
        "  Cached calls: {:?} ({:.2}ms per call)",
        cached_duration,
        cached_duration.as_millis() as f64 / iterations as f64
    );
    println!(
        "  Uncached calls: {:?} ({:.2}ms per call)",
        uncached_duration,
...
async fn monitoring_example() {
    ...
    for prompt in &prompts {
        ...
        if result.is_none() {
            println!("Cache miss for '{prompt}' - making LLM call");
            let generations = expensive_llm_call(prompt, "gpt-4o-mini").await;
            cache
                .aupdate(prompt, llm_string, generations)
                .await
