...
use ferriclink_core::{
    ...
    example_selectors::{
        BaseExampleSelector, Example, LengthBasedExampleSelector,
...
async fn length_based_example() {
    ...
    for (i, example) in selected.iter().enumerate() {
        println!(
            "  {}. Input: {}",
            i + 1,
            example.get("input").unwrap_or(&"N/A".to_string())
        );
        println!(
            "     Output: {}",
            example.get("output").unwrap_or(&"N/A".to_string())
        );
    }
    ...
    println!(
        "With 20 word limit: {} examples selected",
        short_selected.len()
    );
    ...
    println!(
        "With 100 word limit: {} examples selected",
        long_selected.len()
    );
}
...
async fn semantic_similarity_example() {
    ...
    let test_queries: Vec<Example> = vec![
        ...
        .cloned()
        .collect(),
        [("input".to_string(), "How do computers learn?".to_string())]
            .iter()
            .cloned()
            .collect(),
        [("input".to_string(), "Explain image recognition".to_string())]
            .iter()
            .cloned()
            .collect(),
    ];
    ...
    for (i, query) in test_queries.iter().enumerate() {
        println!(
            "\n--- Query {}: {} ---",
            i + 1,
            query.get("input").unwrap_or(&"N/A".to_string())
        );
...
async fn max_marginal_relevance_example() {
    ...
    let query: Example = [(
        "input".to_string(),
        "What is machine learning and AI?".to_string(),
    )]
    .iter()
    .cloned()
    .collect();
...
async fn error_handling_example() {
    ...
    let mut semantic_selector =
        SemanticSimilarityExampleSelector::new(Box::new(vectorstore), 3, None, None, None);
...
async fn performance_comparison() {
    ...
    let input = [(
        "input".to_string(),
        "AI and machine learning concepts".to_string(),
    )]
    .iter()
    .cloned()
    .collect();
    ...
    println!("Length-based is {speedup:.1}x faster than semantic similarity",);
}
