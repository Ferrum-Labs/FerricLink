...
use ferriclink_core::structured_query::{
    Comparator, Comparison, Expr, Operation, Operator, StructuredQuery, Visitor, builders,
};
use serde_json::json;
...
impl Visitor for SqlVisitor {
    ...
    fn visit_structured_query(
        ...
        if let Some(filter) = &structured_query.filter {
            ...
            sql = format!(
                "SELECT * FROM documents WHERE content LIKE '%{}%' AND {}",
                structured_query.query, filter_sql
            );
        }
...
impl Visitor for MongoVisitor {
    ...
    fn visit_comparison(&self, comparison: &Comparison) -> Result<Self::Output, String> {
        ...
        match comparison.comparator {
            ...
            Comparator::Contain => Ok(
                json!({ field: { "$regex": format!(".*{}.*", value.as_str().unwrap_or("")), "$options": "i" } }),
            ),
            Comparator::Like => {
                Ok(json!({ field: { "$regex": value.as_str().unwrap_or(""), "$options": "i" } }))
            }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let complex_query =
        StructuredQuery::new("data science".to_string(), Some(complex_filter), Some(5));
    ...
    println!(
        "Simple MongoDB: {}",
        serde_json::to_string_pretty(&mongo_simple)?
    );

    let mongo_filtered = filtered_query.accept(&mongo_visitor)?;
    println!(
        "Filtered MongoDB: {}",
        serde_json::to_string_pretty(&mongo_filtered)?
    );

    let mongo_complex = complex_query.accept(&mongo_visitor)?;
    println!(
        "Complex MongoDB: {}",
        serde_json::to_string_pretty(&mongo_complex)?
    );
    println!();
